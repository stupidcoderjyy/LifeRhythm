...
void WidgetFactory::setSource(NBT *nbt) {
    if (state != Empty) {
        throw Error("WidgetFactory::setSource required state:Empty");
    }
    if (!nbt) {
        throw Error("WidgetFactory::setSource null nbt");
    }
    source = nbt;
}
...
QWidget* WidgetFactory::apply(QWidget *parent, QWidget *target) noexcept {
    try {
        if (state != Ready) {
            throw Error("WidgetFactory::apply factory '" + id + "' is not in ready state");
        }
...
StandardWidget* WidgetFactory::parseWidgetType(bool& builtIn, NBT *nbt) {
    ...
    if (nbt->contains("loader", Data::STRING)) {
        ...
        if (!loader) {
            throw Error("factory not found '" + loc.toString() + "'");
        }
    }
    ...
    if (!stdWidget) {
        throw Error("unknown type '" + type + "' in widget '" + id + "'");
    }
...
void WidgetFactory::parseChildren(NBT *nbt) {
    ...
    for (auto*& child : arr) {
        if (child->type == Data::STRING) {
            ...
            if (!factory) {
                throw Error("failed to find child loader '" + childId + "'");
            }
            switch (factory->state) {
                case Broken:
                    throw Error("child loader '" + childId + "' is broken");
                case Parsing:
                    throw Error("child loader '" + childId + "' is still in parsing");
                case Empty:
                    factory->parse();
                    break;
                default:
...
void WidgetFactory::parseLayout(NBT *nbt) {
    ...
    if (!nbt->contains("type", Data::STRING)) {
        throw Error("missing layout type");
    }
...
Qt::Alignment WidgetFactory::parseAlign(const QString &alignment) {
    ...
    for (auto& f : flags) {
        if (alignments.contains(f)) {
            ...
            throw Error("invalid alignment flag '" + f + "'");
        }
    }
    return result;
}
...
QSizePolicy::Policy WidgetFactory::parsePolicy(const QString &name) {
    ...
    throw Error("invalid size policy:" + name);
}
...
QMargins WidgetFactory::parseMargins(ArrayData *array) {
    ...
    if (elements.size() != 4) {
        throw Error("invalid margin array");
    }
    int arr[4];
    for (int i = 0 ; i < 4 ; i ++) {
        if (elements[i]->type != Data::INT) {
            throw Error("invalid data type in margin array");
        }
...
void WidgetFactory::parseModel(const NBT* nbt) {
    if (nbt->contains("model", Data::STRING)) {
        ...
        if (!d) {
            throw Error("failed to find widget data '" + nbt->getString("model") + "'");
        }
