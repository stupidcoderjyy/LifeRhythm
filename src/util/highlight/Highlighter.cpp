...
int highlight::TokenSingle::type() {
    return ch;
}

Token::Op highlight::TokenSingle::onMatched(const QString &lexeme, Input *input, Context* ctx) {
    ch = lexeme.at(0).cell();
    return Everything;
}
...
Lexer::Lexer(const Identifier& styleGroupLoc, int statesCount, int startState):
    ...
    if (!styleGroup) {
        throwInFunc("style group not found " + styleGroupLoc.toString());
    }
}
