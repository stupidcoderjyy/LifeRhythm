...
public:
    explicit SyntaxAnalyzer(const QString& file);
    NBT* run();
...
public:
    explicit PropertyRoot(SyntaxAnalyzer *parser);
    void onReduced(Production *p, Property **properties) override;
private:
...
private:
    ...
    void reduce3(PropertyTerminal* p0,PropertyArr* p1,PropertyTerminal* p2); //element → [ arr ]
    void reduce4(PropertyTerminal* p0,PropertyCompound* p1,PropertyTerminal* p2); //element → { compound }
};
