...
StringNbtParser::StringNbtParser(const QString& file) {
    auto* input = CompilerInput::fromFile(file);
    lexer = new StringNbtLexer(input);
...
StringNbtParser::~StringNbtParser() {
...
void StringNbtParser::initActions() {
    actions[0][1] = SHIFT | 2;
    actions[1][0] = ACCEPT;
...
void StringNbtParser::initOthers() {
    terminalRemap[128] = 3;
    terminalRemap[129] = 4;
    terminalRemap[130] = 5;
    terminalRemap[131] = 6;
...
void StringNbtParser::initGrammar() {
...
PropertyRoot::PropertyRoot(StringNbtParser *parser) : parser(parser) {}
