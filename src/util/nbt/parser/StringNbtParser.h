...
#ifndef LIFERHYTHM_STRINGNBTPARSER_H
#define LIFERHYTHM_STRINGNBTPARSER_H
...
private:
    ...
    StringNbtLexer* lexer{};
    std::vector<Symbol*> symbols{};
    NBT* result{};

public:
    explicit StringNbtParser(const QString& file);
    NBT* run();
    virtual ~StringNbtParser();
private:
    void initActions();
    void initGoTo();
    void initOthers();
...
private:
    StringNbtParser* parser;
public:
    explicit PropertyRoot(StringNbtParser *parser);
    void onReduced(Production *p, Property **properties) override;
private:
